...
<div class="container-fluid searchresults" style="margin-top: 70px;">
  ...
  <div class="row">
            ...
            <div class="dropdown pathdropdown" style="display:inline-block;">
                ...
                <i class="glyphicon glyphicon-tasks"></i>
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <?php } else { ?>
                        <li class="small"><?php $cmd = "{\"action\": \"crawl\", \"path\": \"".$fullpath."\", \"index\": \"".$esIndex."\", \"adaptivebatch\": \"true\"}"; ?><a onclick='runCommand(<?php echo $cmd; ?>);' href="#_self"><span style="color:orange;font-weight:bold"><i class="glyphicon glyphicon-repeat"></i> re-crawl rootpath (OVERWRITE INDEX)</span></a></li>
                        ...
                        <li class="small"><?php $cmd = "{\"action\": \"updatedirsizes\", \"index\": \"".$esIndex."\", \"path\": \"".$fullpath."\"}"; ?><a onclick='runCommand(<?php echo $cmd; ?>);' href="#_self"><i class="glyphicon glyphicon-folder-open"></i> update dir sizes (non-recursive)</a></li>
                        <li class="small"><?php $cmd = "{\"action\": \"updatedirsizes\", \"index\": \"".$esIndex."\", \"recursive\": \"true\"}"; ?><a onclick='runCommand(<?php echo $cmd; ?>);' href="#_self"><i class="glyphicon glyphicon-folder-open"></i> update dir sizes (recursive)</a></li>
            ...
            <div class="dropdown">
                ...
                <button title="tags" class="btn btn-default dropdown-toggle btn-xs" type="button" data-toggle="dropdown"><?php if ($tags) { echo $tags; } else { echo "<i class=\"glyphicon glyphicon-tag\" style=\"color:darkgray;\"></i> "; } ?>
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li class="divider"></li>
                        <?php if ($result['_type'] == 'directory') { ?>
                        <li class="small" onclick="$('#tag_<?php echo $result['_id']; ?>').val('tagall_subdirs_norecurs'); document.getElementById('loading').style.display='block'; $.ajax({type:'POST',url:'tagfiles.php',data: $('#changetag_<?php echo $result['_id']; ?>').serialize(),success: function() { location.reload(); } }); return false;"><a href="#"><i class="glyphicon glyphicon-folder-open" style="color:gray"></i> <span style="color:gray">Apply tags to subdirs (non-recursive)</span></a></li>
                        <li class="small" onclick="$('#tag_<?php echo $result['_id']; ?>').val('tagall_files_norecurs'); document.getElementById('loading').style.display='block'; $.ajax({type:'POST',url:'tagfiles.php',data: $('#changetag_<?php echo $result['_id']; ?>').serialize(),success: function() { location.reload(); } }); return false;"><a href="#"><i class="glyphicon glyphicon-file" style="color:gray"></i> <span style="color:gray">Apply tags to files (non-recursive)</span></a></li>
              ...
              <div class="dropdown pathdropdown" style="display:inline-block;">
                  <button title="analytics" class="btn btn-default dropdown-toggle btn-xs file-btns" type="button" data-toggle="dropdown"><i class="glyphicon glyphicon-stats"></i>
                      ...
                      <ul class="dropdown-menu">
                          ...
                          <li class="small"><a href="timechange.php?index=<?php echo $esIndex; ?>&amp;index2=<?php echo $esIndex2; ?>&amp;path=<?php echo rawurlencode($file['path_parent']); ?>&amp;filter=<?php echo $_COOKIE['filter']; ?>&amp;mtime=<?php echo $_COOKIE['mtime']; ?>"><i class="glyphicon glyphicon-time"></i> time change</a></li>
                        </ul>
                  </div>
                  <div class="dropdown pathdropdown" style="display:inline-block;">
    ...
    <!-- end show comparison items -->
        ...
        if (count($extra_fields) > 0) {
          foreach ($extra_fields as $key => $value) { ?>
              <td class="highlight"><?php echo (is_array($file[$key])) ? nl2br(implode("\n",$file[$key])) : $file[$key]; ?></td>
          <?php }
          } ?>
      </tr>
      <?php
